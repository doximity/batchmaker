...
RSpec.describe Batcher do
  ...
  it "call actions with batch of defined sizes" do
    batcher = described_class.new("test", 10, 1, logger, &action)
  ...
  it "calls actions respecting max size when ticking" do
    batcher = described_class.new("test", 10, 0.05, logger, &action)
  ...
  it "handles errors properly" do
    ...
    batcher = described_class.new("test", 5, 1, logger,) do |batch|
  ...
  it "calls action with remaining items when shutting down" do
    batcher = described_class.new("test", 10, 10, logger, &action)
    batcher << 1
    batcher.shutdown!
